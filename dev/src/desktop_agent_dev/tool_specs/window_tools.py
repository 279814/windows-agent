...
def register_window_tools(registry: ToolRegistry, services: Any) -> None:
    ...
    registry.register(ToolSpec(name="window_close", kind="window", params_schema={"type": "object", "properties": {"name": {"type": "string"}}, "required": ["name"]}, result_schema=RESULT_SCHEMAS["input"], permission="window_close", executor=window_close, description="Close a window or app. High-risk; use only with explicit user intent. Success follows the backend close result.", param_description="name: window to close.", result_description="Standard input envelope with close details, close strategy, backend response, and exit code.", input_examples=[{"name": "main"}], output_examples=[{"ok": True, "tool": "window_close", "message": "ok", "data": {"action": "window_close", "ok": True, "detail": "Closed.", "payload": {"name": "main", "target_window": "main", "close_strategy": "backend.close_app", "backend_response": ["Closed.", 0], "exit_code": 0}}}], safety_notes="High risk; gated by the safety service.", implementation_notes="Delegates to backend close_app().", backend_method="close_app"))
