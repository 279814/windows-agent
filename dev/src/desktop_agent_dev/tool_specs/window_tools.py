...
WINDOW_CLOSE_OUTPUT_EXAMPLES = [
    {
        ...
        "data": {
            ...
            "payload": {
                ...
                "target_window": "计算器",
                "matched_by": "handle",
                "close_strategy": "native.close_window",
                "requested_close_path": "wm_close_native",
                "backend_response": ["Closed 计算器.", 0],
                "exit_code": 0,
                "post_close_verified": True,
                "outcome": "execution_succeeded",
                "target_handle": 13502676,
                "target_pid": 4968,
                "verification_mode": "handle_removed",
            },
        },
        "error": None,
    }
...
WINDOW_MINIMIZE_OUTPUT_EXAMPLES = [
    {
        ...
        "data": {
            ...
            "ok": True,
            "detail": "计算器 minimized.",
            "payload": {
                "before_target_window": "计算器",
                "before_window": {"name": "计算器", "status": "MAXIMIZED", "handle": 3017860, "window_title": "计算器"},
                "before_status": "MAXIMIZED",
                "before_handle": 3017860,
                "target_window": "计算器",
                "after_window": None,
                "after_status": None,
                "after_handle": None,
                ...
                "target_instance_handle": 3017860,
                "target_instance_pid": 4968,
                "target_handle_present_after": False,
                "target_window_after_matches_target": False,
                "active_window_after_matches_target": False,
                "active_window_after": {"name": "计算器", "status": "MAXIMIZED", "handle": 2230962, "window_title": "计算器"},
                "native_is_iconic": False,
...
WINDOW_RESTORE_OUTPUT_EXAMPLES = [
    {
        ...
        "data": {
            ...
            "ok": True,
            "detail": "Restored 计算器 window via native handle fallback.",
            "payload": {
                "before_target_window": "计算器",
                "before_window": {"name": "计算器", "status": "MINIMIZED", "handle": 4983846, "window_title": "计算器"},
                "before_status": "MINIMIZED",
                "before_handle": 4983846,
                "target_window": "计算器",
                "after_window": {"name": "计算器", "status": "MAXIMIZED", "handle": 4983846, "window_title": "计算器"},
                "after_status": "MAXIMIZED",
                "after_handle": 4983846,
                "verified": True,
                "active_window_after": {"name": "计算器", "status": "MAXIMIZED", "handle": 4983846, "window_title": "计算器"},
                "verification_mode": "native_zoomed_visible",
                "native_is_iconic": False,
                "native_is_zoomed": True,
                "target_visible_after": True,
                "backend_response": ["Restored 计算器 window via native handle fallback.", 0],
                "backend_response_detail": "Restored 计算器 window via native handle fallback.",
                "backend_response_code": 0,
...
def register_window_tools(registry: ToolRegistry, services: Any) -> None:
    ...
    registry.register(ToolSpec(name="window_launch", kind="window", params_schema={"type": "object", "properties": {"name": {"type": "string"}}, "required": ["name"]}, result_schema=RESULT_SCHEMAS["input"], permission="launch_app", executor=window_launch, description="Launch an application window. Use this when the workflow starts from a named desktop app.\n\nParameters: name identifies the app or window to launch.\nReturns: standard input envelope with launch verification details. The action field is normalized to window_launch even though the executor reuses the app-launch pipeline.\nSafety: may start external applications; gated by the safety service.\nBehavior note: payload reports the resolved target, verification source, before/after window counts, verification_attempts, and new-instance signals. New-instance detection uses raw window diff plus target verification so modern app shells are handled more reliably, but inferred instance creation may still leave new_instance_handles/new_instance_pids empty when no specific handle can be tied to the new shell confidently.\nExamples: {\"name\": \"calc\"}.", param_description="name: app or window to launch.", result_description="Standard input envelope with launch verification details, including requested target, matched target, detected window name, verification_attempts, new_instance_detected details, verification_status, and backend launch response. Inferred instance creation may report empty new_instance_handles/new_instance_pids.", input_examples=[{"name": "calc"}], output_examples=WINDOW_LAUNCH_OUTPUT_EXAMPLES, safety_notes="May start external applications; gated by the safety service.", implementation_notes="Wraps the launch executor, preserves the launch verification payload, and normalizes the action name to window_launch for MCP clients."))
