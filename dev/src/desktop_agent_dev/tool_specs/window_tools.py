...
def register_window_tools(registry: ToolRegistry, services: Any) -> None:
    ...
    registry.register(ToolSpec(name="window_close", kind="window", params_schema={"type": "object", "properties": {"name": {"type": "string"}}, "required": ["name"]}, result_schema=RESULT_SCHEMAS["input"], permission="window_close", executor=window_close, description="Close a target window or application. This is a high-risk command and should only be used when the user intent is explicit.\n\nParameters: name identifies the window to close.\nReturns: standard input envelope with close details. The payload includes the close strategy and backend response so clients can distinguish safety blocking from backend capability gaps.\nSafety: high risk; gated by the safety service.\nExamples: {\"name\": \"main\"}.", param_description="name: window to close.", result_description="Standard input envelope with close details. Payload includes target_window, close_strategy, focus_response (when available), and backend_response.", input_examples=[{"name": "main"}], output_examples=[{"ok": True, "tool": "window_close", "message": "ok", "data": {"action": "window_close", "ok": True, "detail": "closed:C:\\Users\\lenovo\\Desktop\\prompt.txt - Sublime Text", "payload": {"name": "C:\\Users\\lenovo\\Desktop\\prompt.txt - Sublime Text", "target_window": "C:\\Users\\lenovo\\Desktop\\prompt.txt - Sublime Text", "close_strategy": "focus_app+alt+f4", "focus_response": "focused:Sublime Text", "backend_response": "closed:Sublime Text"}}}, {"ok": False, "tool": "window_close", "message": "ok", "data": {"action": "window_close", "ok": False, "detail": "close:C:\\Users\\lenovo\\Desktop\\prompt.txt - Sublime Text", "payload": {"name": "C:\\Users\\lenovo\\Desktop\\prompt.txt - Sublime Text", "target_window": "C:\\Users\\lenovo\\Desktop\\prompt.txt - Sublime Text", "close_strategy": "unavailable", "backend_response": None}}}], safety_notes="High risk; gated by the safety service.", implementation_notes="Delegates to the executor close path and falls back to Alt+F4 when the backend lacks close_app()."))
